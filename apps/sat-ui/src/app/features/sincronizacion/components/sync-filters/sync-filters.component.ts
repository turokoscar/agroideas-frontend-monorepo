    ...
    <div class="flex flex-wrap gap-3 items-end">
      ...
      <div class="flex flex-col gap-1">
        ...
        <input id="codDispositivo" type="text" [(ngModel)]="filtro().codDispositivo" (change)="filtroChange.emit()" placeholder="Código dispositivo"
          class="px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 w-40"/>
      </div>
      <div class="flex flex-col gap-1">
        ...
        <select id="txtResultado" [(ngModel)]="filtro().txtResultado" (change)="filtroChange.emit()"
          class="px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
